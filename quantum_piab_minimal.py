"""
Lightweight Quantum Particle in a Box (PIAB) Streamlit Application
===================================================================
...
- Data export to CSV (no pandas required for export)

Author: Quantum Mechanics Toolbox
Date: 2025

OPTIMIZED: Minimal dependencies, no PyArrow, no openpyxl
"""
...
# ============================================================================
# HELPER FUNCTION FOR TABLE DISPLAY (no pandas needed)
# ============================================================================

def display_simple_table(data_dict):
    """Display data as simple text table without pandas."""
    st.write(data_dict)
...
if app_mode == "📊 Wave Function & Probability":
    ...
    if view_type == "Single State":
        ...
        with col1:
            ...
            for x_i, psi_i in zip(x_test, psi_vals):
                st.write(f"ψ({x_i:.4f}) = {psi_i:.6f}")
        
        with col2:
            prob_vals = calculate_probability_density(x_test, n, L)
            st.subheader("Probability Density")
            for x_i, prob_i in zip(x_test, prob_vals):
                st.write(f"|ψ({x_i:.4f})|² = {prob_i:.6f}")
        ...
        with col3:
            ...
            if abs(norm - 1.0) < 1e-4:
                st.success("✓ Properly normalized!")
            else:
                st.warning(f"Error: {abs(norm - 1.0):.2e}")
...
# ============================================================================
    ...
    try:
        ...
        with col3:
            if probability > 0:
                st.metric("Odds", f"1 in {1/probability:.1f}")
            else:
                st.metric("Odds", "∞")
...
# ============================================================================
    ...
    with col2:
        ...
        st.metric("⟨p⟩ (Average Momentum)", "0.000000 a.u. (symmetry)")
    ...
    if results['satisfies_heisenberg']:
        ...
        st.error("❌ Heisenberg principle violated")
...
# ============================================================================
    ...
    if state_type == "Eigenstate (Stationary)":
        ...
        ax.set_title(f'Stationary State: n = {n}', fontsize=13, fontweight='bold')
        ax.set_xlabel('Position x (a.u.)')
        ...
        if n1 == n2:
            st.warning("n₁ and n₂ must be different!")
        else:
            E1 = calculate_energy(n1, L)
            E2 = calculate_energy(n2, L)
            omega = (E2 - E1) / HBAR
            ...
            st.info(
                ...
                f"• ΔE = {E2-E1:.6f} a.u.\n"
                f"• ω = {omega:.6f} rad/a.u.\n"
                f"• Period: T = {period:.6f} a.u."
            )
...
# ============================================================================
    ...
    st.markdown("""
    The 1D PIAB model explains electronic structure of conjugated molecules
    where π-electrons are delocalized across carbon chains.
    """)
    ...
    with col1:
        ...
        for i in range(1, num_carbons + 1):
            if i == result['n_HOMO']:
                st.write(f"**HOMO: ψ_{i}** (filled)")
            elif i == result['n_LUMO']:
                st.write(f"**LUMO: ψ_{i}** (empty)")
            elif i < result['n_HOMO']:
                st.write(f"ψ_{i} (filled)")
            else:
                st.write(f"ψ_{i} (empty)")
    
    with col2:
        st.subheader("UV-Vis Spectroscopy")
        st.write(f"**HOMO Energy:** {result['E_HOMO_eV']:.4f} eV")
        st.write(f"**LUMO Energy:** {result['E_LUMO_eV']:.4f} eV")
        st.write(f"**Gap (ΔE):** {result['gap_eV']:.4f} eV")
        st.write(f"**Wavelength:** {result['lambda_nm']:.1f} nm")
        st.write(f"**Region:** {result['color_region']}")
    ...
    with st.expander("🧬 Chemistry Insights"):
        ...
        1. **Box Length**: Conjugation length increases → larger effective box
        
        2. **Energy Levels**: E_n ∝ n²/L² → lower energies with larger L
        
        3. **HOMO-LUMO Gap Shrinks**: As L increases, ΔE decreases
        
        4. **Red Shift**: λ = hc/ΔE → smaller gap means longer wavelength
        
        5. **Color Emerges**:
           - UV absorption → colorless
           - Visible absorption → colored
           - Complementary color observed
        
        ### β-Carotene Example
        - 11 conjugated bonds (C₄₀)
        - Absorbs blue-green (~450-500 nm)
        - Appears **orange** 🥕
        """)
...
# ============================================================================
    ...
    with col1:
        n = st.slider("Quantum Number (n)", min_value=1, max_value=10, value=1)
        export_type = st.radio("Export Type:", ["CSV", "JSON", "TXT"])
    ...
    with col2:
        st.info(
            ...
            f"• L = {L} a.u.\n"
            f"• Points: {num_points}\n"
            f"• E = {calculate_energy(n, L):.4f} a.u."
        )
    ...
    with col3:
        st.write("**|ψ(x)|²**")
        for i in range(min(5, num_points)):
            st.write(f"{prob[i]:.6f}")
        if num_points > 5:
            st.write("...")
    
    st.info(f"Total rows: {len(x)}")
    
    if export_type == "CSV":
        csv_data = "position_x,wavefunction_psi,probability_density\n"
        for xi, psi_i, prob_i in zip(x, psi, prob):
            csv_data += f"{xi:.10f},{psi_i:.10f},{prob_i:.10f}\n"
        ...
        json_data = "[\n"
        for xi, psi_i, prob_i in zip(x, psi, prob):
            json_data += f'  {{"x": {xi:.10f}, "psi": {psi_i:.10f}, "prob": {prob_i:.10f}}},\n'
        ...
        txt_data = f"Quantum State Data: n={n}, L={L}\n"
        txt_data += "="*50 + "\n\n"
        txt_data += f"{'Position x':<15} {'ψ(x)':<15} {'|ψ(x)|²':<15}\n"
        txt_data += "-"*50 + "\n"
        for xi, psi_i, prob_i in zip(x, psi, prob):
            txt_data += f"{xi:<15.6f} {psi_i:<15.6f} {prob_i:<15.6f}\n"
        
        st.download_button(
            label="📥 Download TXT",
            data=txt_data,
            file_name=f"quantum_state_n{n}_L{L:.1f}.txt",
            mime="text/plain"
        )
...
Interactive tool for quantum mechanics:
...
**Constants:** ℏ = 1.0, m = 1.0 (atomic units)
